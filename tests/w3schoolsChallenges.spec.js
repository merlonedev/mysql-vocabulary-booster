...
describe("Desafios iniciais", () => {
  ...
  beforeAll(async () => {
    ...
    sequelize = new Sequelize(
      ...
      { host: process.env.HOSTNAME, dialect: "mysql" }
    );
  });
  ...
  describe("17 - Crie uma TRIGGER que, a cada nova inserção realizada na tabela `orders`, insira automaticamente a data atual na coluna `OrderDate`", () => {
    it("Verifica o desafio 17", async () => {
      ...
      const createTriggerQuery = /CREATE TRIGGER.*END/is.exec(
        challengeQuery
      )[0];
      ...
      const result = await sequelize.query(
        `SELECT CustomerID, EmployeeID, OrderDate, ShipperID FROM orders WHERE CustomerID = 4;`,
        { type: "SELECT" }
      );
